...
# # 子进程要执行的代码
# def run_proc(name):
#     print('Run child process %s (%s)...------------------------------------------------------------> SON' % (name, os.getpid()))
#
# if __name__=='__main__':
#     print('Parent process %s.' % os.getpid())
#     p = Process(target=run_proc, args=('test',))
#     print('Child process will start.---0')
#     p.start()
#     print('---------------------------->A')
#     p.join()  # join()方法可以等待子进程结束后再继续往下运行，通常用于进程间的同步
#     print('Child process end.---------------------------->B')


import DIC_Speckle_Quality_Assessment as SQA
...
plt.figure()
a = io.imread(r'C:\Users\Lenovo\Desktop\PyLearning\My_Files\Monterey.jpg', as_gray=True)  # read in pics
# monterey = SQA.Speckle(a)
plt.subplot(3,2,1)
